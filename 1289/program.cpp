...
public:
    int minFallingPathSum(std::vector<std::vector<int>>& grid) {
        ...
        for (size_t i = 0; i < first_row.size(); ++i) {
            int sum = first_row[i] + subMinFallingPathSum(i, grid, memo);
            if (sum < smallest_sum) {
                smallest_sum = sum;
            }
        }
    ...
    int subMinFallingPathSum(size_t last_smallest_index, std::vector<std::vector<int>> grid, std::vector<std::vector<int>>& memo) {
        if (grid.size() == 0) return 0;

        if (memo[0][last_smallest_index] != INT_MAX) {
            // Return memoized value if it's already computed
            return memo[0][last_smallest_index];
        }
        ...
        memo[0][last_smallest_index] = smallest_sum;
...
int main(){
    ...
    std::vector<std::vector<int>> grid { std::vector<int> { 1,2,3 },
